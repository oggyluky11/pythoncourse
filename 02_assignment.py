...
def exercise02():
    ...
    for index in range(0,len_animals):
        print(animals[index])
...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    # This exercise function receives a list of temperatures and a dictionary of temperature data where the key is an arbitrary sequential number and the value is the temperature 
    # and a,b,c,d and e are each a single temperature reading
    # To Do:
    ...
    samples.extend(temperatures[4::5])
...
def exercise05(n):
    ...
    return exercise05(n-1)*n if n > 0 else 1
...
def exercise06(n):
     # This function will receive an arbitrary list of numbers of arbitrary size and find the average of those numbers. The size of the list may vary. Find the method that requires the  least amount of code. 
     # Return back the length, sum of list and average of list
...
def exercise07(n):
    ...
    return False if len(n) == len(set(n)) else True
...
def exercise09():
    # Compile a list of 11 random URLs of dog pics. You will simply APPEND the dogs list with URLs of pics so that the list contains 11 URLs. 
    # This means you will extract the URL and insert into the list dogs.
